...
class PSQLStorage(Storage):
    ...
    def update_task(self,
        ...
        # как-то не красиво, но реализовал
        cur = self.con.cursor()
        if description:
            cur.execute(
            f"""
            ...
            cur.execute(
            f"""
                UPDATE tasks SET due_date='{due_date}' WHERE id={task_id}
            """
            )
        self.con.commit()
...
class TasksListBuilder:
    ...
    def build(self, user_id: int, list_id: int) -> TasksList:
        ...
        for task_db_id, description, status, created_ts, due_date_ts, _, _, _ in related_tasks:
            # TODO: proper init
            # я поменял чуток init у Task класса и тут добавил статус. а то при заходе в существующую
            # учетку статусы все слетали и становились новыми, хотя в базе было не так
            task = Task(description, storage=self.storage, db_id=task_db_id, status=status)
...
# Может надо как-то в отдельный модуль сделать разные сторажи? или мы в итоге только постгрес оставим??
